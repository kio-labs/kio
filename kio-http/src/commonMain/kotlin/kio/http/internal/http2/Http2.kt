...
object Http2 {
  val CONNECTION_PREFACE: ByteString = "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n".encodeToByteString()
  ...
  private val BINARY =
    Array(256) {
      it.toString(radix = 2).padStart(8, '0')
    }
  ...
  fun frameLog(
    ...
    return "$direction " +
            "0x${streamId.toUInt().toString(16).padStart(8, '0')} " +
            length.toString().padStart(5, ' ') + " " +
            formattedType.padEnd(13, ' ') + " " +
            formattedFlags
  }
  ...
  internal fun formattedType(type: Int): String =
    if (type < FRAME_NAMES.size) FRAME_NAMES[type] else "0x${(type and 0xff).toString(16).padStart(2, '0')}"
