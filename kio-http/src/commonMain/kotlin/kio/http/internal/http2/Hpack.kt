...
import kio.http.internal.http2.Header.Companion.TARGET_AUTHORITY
import kio.http.internal.http2.Header.Companion.TARGET_METHOD
import kio.http.internal.http2.Header.Companion.TARGET_PATH
import kio.http.internal.http2.Header.Companion.TARGET_SCHEME
import kio.http.internal.http2.Header.Companion.RESPONSE_STATUS
...
object Hpack {
    ...
    constructor(
        ...
        private val source: AsyncSource = source.buffered()
        ...
        private fun evictToRecoverBytes(bytesToRecover: Int): Int {
            ...
            if (bytesToRecover > 0) {
                ...
                dynamicTable.copyInto(
                    dynamicTable,
                    nextHeaderIndex + 1 + entriesToEvict,
                    nextHeaderIndex + 1,
                    nextHeaderIndex + 1 + headerCount,
                )
        ...
        private suspend fun readLiteralHeaderWithoutIndexingIndexedName(index: Int) {
            val name = getName(index)
            val value = readByteString()
            addHeader(Header(name, value))
        }

        private suspend  fun readLiteralHeaderWithoutIndexingNewName() {
            val name = checkLowercase(readByteString())
            val value = readByteString()
            addHeader(Header(name, value))
        }

        private suspend fun readLiteralHeaderWithIncrementalIndexingIndexedName(nameIndex: Int) {
            val name = getName(nameIndex)
            val value = readByteString()
            insertIntoDynamicTable(-1, Header(name, value))
        }

        private suspend fun readLiteralHeaderWithIncrementalIndexingNewName() {
            val name = checkLowercase(readByteString())
            val value = readByteString()
            insertIntoDynamicTable(-1, Header(name, value))
        }
        ...
        private fun insertIntoDynamicTable(
            ...
            if (index == -1) { // Adding a value to the dynamic table.
                if (headerCount + 1 > dynamicTable.size) { // Need to grow the dynamic table.
                    ...
                    dynamicTable.copyInto(doubled, dynamicTable.size, 0, dynamicTable.size)
                    nextHeaderIndex = dynamicTable.size - 1
        ...
        private suspend fun readByte(): Int = source.readByte() and 0xff
        ...
        suspend fun readByteString(): ByteString {
            ...
            return if (huffmanDecode) {
                ...
                ByteString(source.readByteArray(length.toInt()))
            }
        }
    ...
    constructor(
        ...
        private fun evictToRecoverBytes(bytesToRecover: Int): Int {
            ...
            if (bytesToRecover > 0) {
                ...
                dynamicTable.copyInto(
                    destination = dynamicTable,
                    destinationOffset = nextHeaderIndex + 1 + entriesToEvict,
                    startIndex = nextHeaderIndex + 1,
                    endIndex = nextHeaderIndex + 1 + headerCount,
                )
                dynamicTable.fill(
                    element = null,
                    fromIndex = nextHeaderIndex + 1,
                    toIndex = nextHeaderIndex + 1 + entriesToEvict,
                )
                nextHeaderIndex += entriesToEvict
            }
            return entriesToEvict
        ...
        private fun insertIntoDynamicTable(entry: Header) {
            ...
            if (headerCount + 1 > dynamicTable.size) { // Need to grow the dynamic table.
                ...
                dynamicTable.copyInto(
                    destination = doubled,
                    destinationOffset = dynamicTable.size,
                    startIndex = 0,
                    endIndex = dynamicTable.size,
                )
                nextHeaderIndex = dynamicTable.size - 1
                dynamicTable = doubled
            }
        ...
        fun writeHeaders(headerBlock: List<Header>, out: Buffer) {
            ...
            for (i in 0 until headerBlock.size) {
                ...
                val name = ByteString(header.name.decodeToString().lowercase().toByteArray())
        ...
        fun writeInt(
            ...
            if (value < prefixMask) {
                out.writeByte((bits or value).toByte())
                return
            }

            // Write the mask to start a multibyte value.
            out.writeByte((bits or prefixMask).toByte())
            value -= prefixMask

            // Write 7 bits at a time 'til we're done.
            while (value >= 0x80) {
                val b = value and 0x7f
                out.writeByte((b or 0x80).toByte())
                value = value ushr 7
            }
    ...
    fun checkLowercase(name: ByteString): ByteString {
        for (i in 0 until name.size) {
            if (name[i] in 'A'.code.toByte()..'Z'.code.toByte()) {
                throw IOException("PROTOCOL_ERROR response malformed: mixed case name: ${name.toByteArray().decodeToString()}")
            }
        }
        return name
    }
